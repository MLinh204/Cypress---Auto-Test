            ...
            cy.randomEmail().then((email) => {
              cy.window().then(win => {
                return new Cypress.Promise((resolve, reject) => {
                  ...
                  expect(response).to.have.property('email', email);
                  expect(response).to.have.property('username', 'Edited Username 1');
                  expect(response).to.have.property('profile_picture');
    ...
    after(function() {
        ...
        cy.window().then(win => {
            return new Cypress.Promise((resolve, reject) => {
                ...
                // Use the saved userBeforeEdit data
                formData.append('id', userBeforeEdit.id);
                formData.append('email', userBeforeEdit.email);
                formData.append('username', userBeforeEdit.username);
                formData.append('password', userBeforeEdit.password);
                
                // If there was a profile picture before, we need to handle it
                // Since we can't restore the exact file, we'll just keep the current one
